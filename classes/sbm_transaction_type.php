...
class sbm_transaction_type {

	private function sbm_create_transaction_type()
	{
		global $wpdb;
		// Insert a record into the sbm_meta table
		$wpdb->insert(  $wpdb->prefix."sbm_meta", array(  'meta_key'=> 'transaction_type' ), array(  '%s' ) );
		
		return $wpdb->insert_id;	
		
	}

	public function sbm_get_transaction_type_data($id)
	{
		global $wpdb;
	...
	public function sbm_get_transaction_type_meta()
	{
		global $wpdb;
	...
	public function sbm_delete_transaction_type($id)
	{
		global $wpdb;
	...
	public function sbm_update_transaction_type()
	{
		global $wpdb;
		global $current_user;
		...
		if( $_POST['ID'] == 'new' )
		{
			$id = $transaction_type_info->sbm_create_transaction_type();
		...
		else
		{
			$id = $_POST['ID'];
						...
						$property_data = array();
						$type = array();
						$property_data['meta_value'] = $implode;
					...
					$wpdb->update( $wpdb->prefix."sbm_meta", $property_data, array( 'ID' => $id ), $type, array( '%d' ) );
					// redirect user back to the view transaction type list page
					//  general_functions.php:     sbm_redirect()
					sbm_redirect('sbm_view_transaction_type_list', 'success_transaction_type');
